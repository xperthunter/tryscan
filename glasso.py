...
eps = 1e-12
lambda_h = 0.2
H = -np.sum(fi * np.log2(fi), axis=1)
H_norm = (H - H.min()) / (H.max() - H.min() + 1e-6)
lh = lambda_h * (1 - (H_norm[:, None] + H_norm[None, :]) / 2))
"""
...
lam = lambda0 + lambda_d * np.exp(-dist / tau)
np.fill_diagonal(lam, 0.0)


n = cov.shape[0]
eps = 1e-6
K = np.linalg.inv(cov + eps * np.eye(n))
K0 = K.copy()
K_prev = K.copy()
max_iter = 100
eta = 0.1
for it in range(max_iter):
	...
	K[off_diag] = np.sign(K[off_diag]) * np.maximum(
		np.abs(K[off_diag]) - eta * lam[off_diag],
		1.0 / (it + 1)**2
	)
	...
	if sign <= 0: logdet = np.inf
	
	cov_re = np.linalg.inv(K)
	cov_diff = np.linalg.norm(cov - cov_re)
	
	diff_K0 = np.linalg.norm(K - K0)
	
	diff_step = np.linalg.norm(K - K_prev)
	
	sparsity = np.sum(np.abs(K) < 0.1) / K.size
	
	print(
		f"{it:3d} | sparsity: {sparsity:.4f} "
		f"logdet: {logdet:.3e} "
		f"||C - K^-1||: {cov_diff:.3e} "
		f"||K  - K0 ||:  {diff_K0:.3e} "
		f" K step: {diff_step:.3e}"
	)
	K_prev = K.copy()
...
im0 = axes[0].imshow(K0, cmap="coolwarm", vmin=vmin, vmax=vmax)
axes[0].set_title("K0")

im1 = axes[1].imshow(K, cmap="coolwarm", vmin=vmin, vmax=vmax)
